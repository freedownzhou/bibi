    ...
    <div class="bh-body">
      <el-row  style="height:28px;border:1px solid #d8d8d8;">
        <el-col style="line-height:28px;background-color:#f4f4f4;text-align: center;cursor: pointer;" >
          <span>账户总资产：<span style="color:#0bb2fe;">{{Number(totalMoney).cut(4)}}</span>{{showCNY ? 'CNY' : 'USD'}}</span>&nbsp;&nbsp;
          <span>可用：<span style="color:#0bb2fe;">{{Number(totalMoney - frozenTotalMoney).cut(4)}}</span>{{showCNY ? 'CNY' : 'USD'}}</span>&nbsp;&nbsp;
          <span>冻结：<span style="color:#0bb2fe;">{{Number(frozenTotalMoney).cut(4)}}</span>{{showCNY ? 'CNY' : 'USD'}}</span>&nbsp;&nbsp;
...
export default {
  ...
  computed: {
    convertToCNY () {
      return this.$store.getters.convertToCNY
    },
    convertToUSD () {
      return this.$store.getters.convertToUSD
    },
    totalMoney () {
      let total = 0
      if (this.showCNY) {
        for (let i in this.tableData) {
          total += this.convertToCNY(this.tableData[i].dictionariesId, this.tableData[i].balance)
        }
      } else {
        for (let i in this.tableData) {
          total += this.convertToUSD(this.tableData[i].dictionariesId, this.tableData[i].balance)
        }
      }
    ...
    frozenTotalMoney () {
      ...
      if (this.showCNY) {
        for (let i in this.tableData) {
          frozenTotal += this.convertToCNY(this.tableData[i].dictionariesId, this.tableData[i].frozenAmount)
        }
      } else {
        for (let i in this.tableData) {
          frozenTotal += this.convertToUSD(this.tableData[i].dictionariesId, this.tableData[i].frozenAmount)
        }
      }
      return frozenTotal.toFixed(2)
    }
  },
  methods: {
    goTo (key) {
    ...
    refreshData () {
      ...
      // this.$http.post('/wallet/selectWalletListUI', {currencyCode: ''}).then(response => {
      //   this.$store.commit('setLastUpdateTime', +new Date())
      //   this.$store.commit('setAssets', response.body.data)
  ...
  created () {
    this.committing = true
    this.$store.dispatch('updateAssets').then(() => {
      this.tableData = this.$store.getters.getSourceAssetsData
      this.committing = false
    }, data => {
      this.committing = false
    })
