<template>
  <div>
    <div class="bh-slider">
      <div>
        <span @click="reduce" class="bh-font_list">{{min}}</span>
      </div>
      ...
      <div>
        <span @click="increase" class="bh-font_list">{{max}}</span>
      </div>
    </div>
    <!--输入控制-->
    <div style="text-align: center;width: 100%;" @click="$refs.input.focus()" class="bh-font_size__small bh-color_dark">
      <div style="text-align: right;display: inline-block;width: 45%;">
        <input ref="input" class="bh-slider_input bh-font_size__small bh-color_dark" v-model="percent2" type="number" maxlength="4" @blur="onBlurForInput"/>
      </div>
