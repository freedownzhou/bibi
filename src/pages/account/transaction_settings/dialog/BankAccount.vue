<template>
  <div class="">
    <!--银行账户设置-->
    <bh-dialog :visible="visible" @updateVisible="$emit('update:visible', false)" :close-button="false" :width="527" :height="449">
      <div slot="body">
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-input  style="width: 290px; height: 34px;margin: 22px 0 2px 0px;" v-model="bankUserName" @focus="bankUserNameFocus=true" clearable disabled></el-input>
...
export default {
  data () {
    return {
      ...
      bankUserName: this.$store.getters.getUser.username,
      bankCard: '',
      confirmBankCard: '',
      bankOfDeposit: '',
      branches: '',
  ...
  created () {
    ...
    if (user.bankcard && user.bankUserName) {
      // 已绑定,本地有数据
      this.bankUserName = user.bankUserName
      this.bankCard = user.bankCard
      this.confirmBankCard = user.bankCard
      this.bankOfDeposit = user.bankOfDeposit
      this.branches = user.branches
    } else if (user.bankcard) {
      // 已绑定,本地无数据
      this.$store.dispatch('encryptConnection', {
        url: '/bank/queryUI'
      }).then(response => {
        ...
        const bankcard = {
          bankUserName: data.bUserName,
          bankCard: data.bBankAccount,
          bankOfDeposit: data.bBankName,
          branches: data.bBankDot
        }
        this.$store.commit('setUser', bankcard)
        this.bankUserName = data.bUserName
        this.bankCard = data.bBankAccount
        this.bankOfDeposit = data.bBankName
        this.branches = data.bBankDot
      }, response => {
        this.resetData()
        switch (response.body.code) {
          case 666:
