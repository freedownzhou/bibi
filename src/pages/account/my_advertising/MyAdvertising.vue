<template>
  <div class="bh-root">
    ...
    <el-row>
      <el-col :span="24">
        <span class="bh-background_normal bh-font_subtitle">我的交易广告</span>
      </el-col>
    </el-row>
    ...
    <el-row style="width: 600px;">
      <el-col :span="24">
        <span class="bh-font_size__big" :class="{'bh-color_standard': selected==='ALL'}" @click="select('ALL')">全部币种</span>
        <span class="bh-font_size__big" :class="{'bh-color_standard': selected==='BTC'}" @click="select('BTC')">BTC</span>
        <span class="bh-font_size__big" :class="{'bh-color_standard': selected==='ETH'}" @click="select('ETH')">ETH</span>
        <span class="bh-font_size__big" :class="{'bh-color_standard': selected==='GXC'}" @click="select('GXC')">GXC</span>
        <span style="cursor: not-allowed" class="bh-font_size__big" :class="{'bh-color_standard': selected==='More'}">{{/BTC|ETH|GXC/.test(currencyCode) ? '' : currencyCode}}</span>
    ...
    <el-row>
      <el-col :span="24">
        <el-checkbox-group v-model="checkedAds" @change="handleCheckedAdsChange">
          <el-table :data="allList" v-loading="loading" border style="width: 100%">
            <el-table-column prop="choice" label="选择" align="center" width="50">
    ...
    <!--广告编辑-->
    <advertisement-editor :advertising="advertising" :visible.sync="advertisementEditor" v-if="advertisementEditor" @success="editSuccess"></advertisement-editor>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    platformCoinDetailList () {
      return this.$store.getters.getPlatformCoinList
    }
  },
  methods: {
    // 全选
    ...
    // 广告编辑成功
    editSuccess () {
      this.advertisementEditor = false
      this.query()
    }
  },
  created () {
