<template>
  <div class="bh-update_phone_lost">
    ...
    <el-row>
      <el-col :span="24">
        <span class="bh-background_normal bh-font_subtitle">登录密码修改</span>
      </el-col>
    </el-row>

    <el-row>
      <el-col :span="4" type="flex">
        <span class="bh-col_1 bh-font_normal">原密码</span>
      </el-col>
      <el-col :span="7">
        <el-input type="password" v-model="oldPassword" placeholder="请输入原密码" @focus="checkOldPasswordFocus" @blur="checkOldPasswordBlur"></el-input>
    ...
    <el-row>
      <el-col :span="4" type="flex">
        <span class="bh-col_1 bh-font_normal">新密码</span>
      </el-col>
      <el-col :span="7">
        <el-input type="password" v-model="newPassword" placeholder="请输入新密码" @focus="checkNewPasswordByFocus" @blur="checkNewPasswordByBlur"></el-input>
        <transition name="el-fade-in-linear">
    ...
    <el-row>
      <el-col :span="4" type="flex">
        <span class="bh-col_1 bh-font_normal">确认密码</span>
      </el-col>
      <el-col :span="7">
        <el-input type="password" v-model="confirmPassword" placeholder="确认密码" @focus="checkConfirmPasswordByFocus" @blur="checkConfirmPasswordByBlur"></el-input>
        <transition name="el-fade-in-linear">
    ...
    <el-row>
      <el-col type="flex" :span="4" >
        <span class="bh-col_1 bh-font_normal">{{user.telephone ? '短信' : '邮箱'}}验证码</span>
      </el-col>
      <el-col :span="7">
        <el-input v-model="captcha" placeholder="请输入验证码" @focus="checkCaptchaByFocus" @blur="checkCaptchaByBlur"></el-input>
        <transition name="el-fade-in-linear">
...
export default {
  ...
  watch: {
    oldPassword () {
      ...
      // 2.原密码格式的校验
      if (!this.$RegExp.password(this.oldPassword)) {
        this.showOldPasswordMsg = true
        this.oldPasswordMsg = '请输入8-20位数字、字母组合'
    ...
    newPassword () {
      ...
      // 2.新密码格式的校验
      if (!this.$RegExp.password(this.newPassword)) {
        this.showNewPasswordMsg = true
        this.newPasswordMsg = '请输入8-20位数字、字母组合'
      ...
      if (this.newPassword === this.oldPassword) {
        this.showNewPasswordMsg = true
        this.newPasswordMsg = '新密码不能与原密码相同'
        this.newPasswordTestResults = 2
        return
      }
  ...
  methods: {
    ...
    commit () {
      this.$store.dispatch('encryptConnectionSSO', {
        url: '/sso/user/updatePassword',
        data: {
          userId: this.user.userId + '',
          originPassword: this.oldPassword,
