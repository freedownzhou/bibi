  ...
  <div class="activity">
    ...
    <div class="zpBox">
      <div class="b b_1">
        ...
        <img src="../assets/images/activity/c1.png" class="b c_1" id="cycle">
        <img src="https://8bihui-webapp.oss-cn-beijing.aliyuncs.com/images/activity/c2.png" class="b c_2" @click="rotate">
      </div>
    </div>
    <!--活动流程-->
    <div class="lcBox">
      <img v-if="step === 1" src="../assets/images/activity/b2_1.png" class="b b_2">
      <img v-else-if="step === 2" src="../assets/images/activity/b2_2.png" class="b b_2">
      <img v-else-if="step === 3" src="../assets/images/activity/b2_3.png" class="b b_2">
      <img v-else-if="step === 4" src="../assets/images/activity/b2_4.png" class="b b_2">
    </div>
    <!--排行榜-->
    <div class="phbBox">
      <img src="../assets/images/activity/b3.png" class="b b_3">
      ...
      <div class="list list1" v-if="isShowPhbList === 1">
        ...
        <p :class="{odd: index % 2 === 1, even: index % 2 === 0}" v-for="(data, index) in findLotteryRecordUIList" :key="index">
      ...
      <div class="list list2" v-if="isShowPhbList === 2">
        ...
        <p :class="{odd: index % 2 === 1, even: index % 2 === 0}" v-for="(data, index) in findInviteRecordUIList" :key="index">
      ...
      <div class="list list3" v-if="isShowPhbList === 3">
        ...
        <p :class="{odd: index % 2 === 1, even: index % 2 === 0}" v-for="(data, index) in findTranNumRecordUIList" :key="index">
          <span v-if="data.isNullData">&nbsp;</span>
          <span v-else>{{index + 1}}</span>
          <span>{{data.userName ? data.userName : '-'}}&nbsp;</span>
    ...
    <div class="ruleBox pb100">
      <img src="../assets/images/activity/b4.png" class="b b_4">
      <!--<div class="title">活动规则</div>-->
      <ul>
        <li>1.活动时间：2019年2月26日 12:00 – 2019年3月5日 24:00;</li>
        <li>2.注册送币活动限新用户（新注册或者注册未中级实名）参与；抽奖活动限邀请2名中级实名用户与交易2天的用户参与;</li>
        <li>3.2019年2月26日 12:00 – 2019年3月5日 24:00，活动期间如果提币，将失去活动二的奖励资格;</li>
        <li>4.活动描述：</li>
        <li>注册奖励/JON充值奖励/交易JON/抽奖/等四种形式均可获得奖励。活动结束时累计交易额排行榜前十名还可获得额外的送币奖励;</li>
        <li>5.奖励说明：iPhone XS/1000元京东购物卡/JON币等奖品;</li>
        <li>6.奖励发放：iPhone XS及1000元京东购物卡需联系客服发放。其它奖励均以JON币种进行发放，JON币种奖励在每天24:00之前进行发放;
          iPhone XS及京东1000元购物卡将在活动结束后的5日内发放;</li>
        <li>7.如果对奖励发放存在疑问，请在奖励发放后5日内联系平台进行处理，过时将不予处理，敬请谅解;</li>
        <li>8.为保证活动公平，一旦发现作弊行为，将取消奖励资格。www.8bihui.com保留对活动的最终解释权;</li>
        <li class="lastLi">风险提示：数字货币是一种高风险的投资方式，请投资者理性判断自己的投资能力，审慎做出投资决策;</li>
      </ul>
      <img src="../assets/images/activity/moreBtn.png" class="moreBtn" @click="moreBtn()">
    </div>
    <!--锚点-->
...
export default {
  ...
  data () {
    return {
      ...
      phbBg: [
        {
          'on': require('../assets/images/activity/top1_on.png'), 'no': require('../assets/images/activity/top1.png')
        },
        {
          'on': require('../assets/images/activity/top2_on.png'), 'no': require('../assets/images/activity/top2.png')
        },
        {
          'on': require('../assets/images/activity/top3_on.png'), 'no': require('../assets/images/activity/top3.png')
        }
      ],
      phb_one: '',
      phb_two: '',
      ...
      // 活动id
      activityId: '1028',
      findLotteryRecordUIList: {}, // 奖励排行榜
      findInviteRecordUIList: {}, // 邀请排行榜
      findTranNumRecordUIList: {} // 交易排行榜
    }
  },
  computed: {
    // 用户参与抽奖资格
  ...
  methods: {
    ...
    findLotteryRecordUI () {
      this.$store.dispatch('encryptConnection', {
        ...
        this.findLotteryRecordUIList = response.body.data
      }, response => {
        this.message.error(response.body.msg)
      })
    },
    ...
    findInviteRecordUI () {
      this.$store.dispatch('encryptConnection', {
        ...
        this.findInviteRecordUIList = response.body.data
      }, response => {
        this.message.error(response.body.msg)
      })
    },
    ...
    findTranNumRecordUI () {
      this.$store.dispatch('encryptConnection', {
        ...
        console.log(response.body.data)
        this.findTranNumRecordUIList = response.body.data
      }, response => {
        this.message.error(response.body.msg)
      })
    },
...
.zpBox img,.lcBox img,.phbBox img,.ruleBox img{position: relative;top:0;left:0;width: 100%;display: block;}
  ...
  /*转盘*/
  .zpBox{background: url("../assets/images/activity/bg1.jpg") no-repeat;background-size: 100%;}
  ...
  /*活动流程*/
  .lcBox{background: url("../assets/images/activity/bg2.jpg") no-repeat;background-size: cover;}
  .lcBox .b_2{width: auto;position: relative;}

  /*排行榜*/
  .phbBox{background: url("../assets/images/activity/bg3.jpg") no-repeat;background-size: cover;}
  ...
  /*活动规则*/
  .ruleBox{clear: both;background: url("../assets/images/activity/bg4.jpg") no-repeat;background-size: cover;}
  ...
  .ruleBox ul{position: absolute;width: 72%;height: auto;left: 0;right: 0;margin: auto;top: 18%;max-width: 1000px;}
  .ruleBox ul li{position: relative;width: 100%;float: left;font-size: 20px;font-weight: 400;color: #fff;line-height: 36px;}
  .ruleBox ul .lastLi{color: #FCE700;margin-top: 30px;}
  .ruleBox .moreBtn{position: absolute;width: 12%;left: 0;right: 0;top: 71%;cursor: pointer;margin: 0 auto;}
