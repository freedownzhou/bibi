    ...
    <main id="particles-js" class="bh-background_main">
      <div class="bh-login">
        ...
        <div class="bh-login_body">
          ...
          <div><div class="bh-font_size__small bh-color_select forget" @click="$router.push({name: 'ForgetPassword'})">忘记密码?</div></div>
    ...
    createWallet () {
      this.$store.dispatch('encryptConnection', {
        ...
        for (let i in list) {
          // 补充地址信息
          if (!list[i].path) {
            this.$http.post('/wallet/createWalletUI', {'currencyCode': list[i].dictionariesId}).then(response => {
            }, response => {})
          }
        }
      }, response => {})
